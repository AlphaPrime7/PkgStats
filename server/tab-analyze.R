...
observeEvent(input$plot_button,{
  ...
  if(isTRUE(input$smoothen)){
    ...
    if (input$plot_freq == "week") {
      ...
      d = d %>% 
        dplyr::group_by(year = lubridate::year(date), week = lubridate::week(date), package) %>% 
        summarise_if(is.numeric, sum)
      d = cbind(d, as.numeric(1))
      names(d)[ncol(d)] <- "control"
      suppressMessages ({
        date = as.data.frame(with(d, aweek::get_date(year = year, week = week, day = control)))
        d = cbind(d,date)
        names(d)[ncol(d)] <- "date"
      })
      ...
      d = d %>%
        dplyr::group_by(package) %>%
        dplyr::transmute(count=cumsum(count), date=date) 
